...
class StatesChart extends React.Component {
    ...
    async componentDidMount() {
      var stateData = await stateOutcomeMoney("Celje");

      var nestedData = d3.nest().key(function(d) {return d.main;}).key(function(d) { return d.sub_cat; }).key(function(d) { return d.sub_sub; }).entries(stateData);
      console.log("Nested: ", nestedData);
      this.main({title: "World Population"}, {key: "World", values: stateData});
    }
    ...
    main(o, data) {
        ...
        var svg = d3.select(this.refs.statesChart).append("svg")
    ...
    render() { 
        return (
            <div ref="statesChart" id="statesChart"></div> 
        );
    }
}
