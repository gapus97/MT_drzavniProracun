...
class StatesChart extends React.Component {
    svg;defaults;opts;grandparent;transitioning;root;formatNumber;x;y;color;fader;treemap;defaults;
    constructor(props) {
        super(props);
      };
       defaults = {
       ...
       x= d3.scaleLinear().domain([0, this.opts.width]).range([0, this.opts.width]);
       y = d3.scaleLinear().domain([0, this.opts.height - this.opts.margin.top - this.opts.margin.bottom])
        .range([0, this.opts.height - this.opts.margin.top - this.opts.margin.bottom]);
      ...
      g1;g;
       ...
       data2 = {
        ...
        //	      .round(false);
            ...
            this.svg = d3.select("#domainDrillDown").append("svg")
              .attr("width", this.opts.width - this.opts.margin.left - this.opts.margin.right)
              .attr("height", this.opts.height - this.opts.margin.bottom - this.opts.margin.top)
              .style("margin-left", -this.opts.margin.left + "px")
              .style("margin.right", -this.opts.margin.right + "px")
              .append("g")
            ...
            this.grandparent.append("rect")
              ...
              .attr("height", this.opts.margin.top);

            this.grandparent.append("text")
              .attr("x", 6)
              .attr("y", 6 - this.opts.margin.top)
              .attr("dy", ".75em");

           this.treemap = d3.treemap()
            .tile(d3.treemapResquarify.ratio(this.opts.height / this.opts.width * 0.5 * (1 + Math.sqrt(5))))
            .size([this.opts.width, this.opts.height])
            .round(false)
            .paddingInner(1);
          }

          var root = d3.hierarchy(this.data2)
        ...
        initialize(root){
          root.x = root.y = 0;
          root.x1 = this.opts.width;
          root.y1 = this.opts.height;
          root.depth = 0;
        }
        ...
        //      ? d.value = d.children.reduce(function(p, v) { return p + this.accumulate(v); }, 0) : d.value;
        //   }
        layout(d) {
            if (d._children) {
         ...
         display(d) {
            ...
            this.g.filter(function(d) { return d._children; })
              .classed("children", true)
              .on("click", this.transition);

            var children = this.g.selectAll(".child")
            ...
            children.append("rect")
              ...
              .call(this.rect)
              .append("title")
              .text(function(d) { return d.data.shortName + " (" + this.formatNumber(d.value) + ")"; });
            ...
            t.append("tspan")
              .attr("dy", "1.0em")
              .text(function(d) { return this.formatNumber(d.value); });

            t.call(this.text);

            this.g.selectAll("rect")
              .style("fill", function(d) { return this.color(d.data.shortName); });
          ...
          transition(d) {
            ...
            var g2 = this.display(d),
              ...
              this.x.domain([d.x0, d.x0 + (d.x1 - d.x0)]);
              this.y.domain([d.y0, d.y0 + (d.y1 - d.y0)]);
          ...
          text(text) {
            text.selectAll("tspan")
              .attr("x", function(d) { return this.x(d.x0) + 6; })
            text.attr("x", function(d) { return this.x(d.x0) + 6; })
              .attr("y", function(d) { return this.y(d.y0) + 3; })
              .style("opacity", function(d) {
                 var w = this.x(d.x1) - this.x(d.x0);
          ...
          text2(text) {
            text.attr("x", function(d) {
               return this.x(d.x1) - this.getComputedTextLength() - 6;
            })
            .attr("y", function(d) { return this.y(d.y1) - 6; })
            .style("opacity", function(d) {
               var w = this.x(d.x1) - this.x(d.x0);
          ...
          rect(rect) {
            rect.attr("x", function(d) { return this.x(d.x0); })
              .attr("y", function(d) { return this.y(d.y0); })
          ...
          name(d) {
            return d.parent ? this.name(d.parent) + " / " + d.data.shortName + " (" + this.formatNumber(d.value) + ")" : d.data.shortName + " (" + this.formatNumber(d.value) + ")";
          }
        ...
        render() {
          return (
              <div ref="statesChart" id="statesChart">{this.updateDrillDown()}</div>
          );
      }
    }
